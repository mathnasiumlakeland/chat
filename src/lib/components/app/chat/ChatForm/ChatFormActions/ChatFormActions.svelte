<script lang="ts">
	...
	let modelSelectionTargetId = $derived.by(() =>
		resolveChatFormModelSelection({
			conversationModel,
			currentSelectedModelId: selectedModelId(),
			isRouter,
			loadedModelIds: modelsStore.loadedModelIds,
			options: modelOptions()
		})
	);

	$effect(() => {
		if (modelSelectionTargetId) {
			void modelsStore.selectModelById(modelSelectionTargetId);
		}
	});
